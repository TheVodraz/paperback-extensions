...
var _Sources = (() => {
  ...
  var parseMangaDetails = (data, mangaId) => {
    ...
    for (const tag of data.genres.split(",")) {
      const label = decode(tag ?? "").trim();
      const id = normalizeGenre(tag);
  ...
  var normalizeGenre = (value) => {
    return decode(value ?? "").toLowerCase().trim().replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
  };
  var parseGenres = (genres) => {
    return (genres ?? "").split(",").map((x) => normalizeGenre(x)).filter(Boolean);
  };
  ...
  var extractMhubAccess = (value) => {
    ...
    if (!source) return "";
    const cookieMatch = /mhub_access=([^;]+)/.exec(source);
    return cookieMatch?.[1] ?? source;
  };
  var extractSetCookieHeader = (headers) => {
    const cookieHeaders = headers?.["Set-Cookie"] ?? headers?.["set-cookie"];
    if (!cookieHeaders) return "";
    const mergedCookies = Array.isArray(cookieHeaders) ? cookieHeaders.join(";") : String(cookieHeaders);
  ...
  var parseSearch = (rows, filters = {}) => {
    const collectedIds = [];
    const searchResults = [];
    const includedGenres = filters.includedGenres ?? [];
    const excludedGenres = filters.excludedGenres ?? [];
    const authorQuery = (filters.authorQuery ?? "").toLowerCase();
    for (const manga of rows) {
      const title = manga.title ?? "";
      const id = manga.slug ?? "";
      const image = manga?.image ? `${MH_CDN_THUMBS_DOMAIN}/${manga.image}` : "";
      const subtitle = manga?.latestChapter ? "Chapter " + manga.latestChapter : "";
      const author = decode(manga.author ?? "");
      const mangaGenres = parseGenres(manga.genres);
      if (!id || !title || collectedIds.includes(manga.id)) continue;
      if (includedGenres.length > 0 && !includedGenres.every((genre) => mangaGenres.includes(genre))) continue;
      if (excludedGenres.some((genre) => mangaGenres.includes(genre))) continue;
      if (authorQuery && !author.toLowerCase().includes(authorQuery) && !decode(title).toLowerCase().includes(authorQuery)) continue;
  ...
  var Mangahub = class {
    constructor() {
      ...
      this.storeMhubAccess = async (value) => {
        const mhubAccess = extractMhubAccess(value);
        if (!mhubAccess) return;
        await this.stateManager.store("mhub_key", mhubAccess);
      };
    ...
    getGraphQLErrorMessage(errors) {
      return errors.map((x) => x?.message ?? `${x}`).join("\n").trim() || "Mangahub request failed.";
    }
    shouldRetryGraphQLError(errors) {
      const message = this.getGraphQLErrorMessage(errors);
    ...
    async executeGraphQL(query, retries = 2, headers = {}) {
      ...
      const payload = this.parseGraphQLResponse(response);
      ...
      return `query {
                    search(x: m01, alt: ${alt ? "true" : "false"}, q: "${escapeGraphQLString(title)}", genre: "${escapeGraphQLString(genre)}", offset:${offset}) {
                      rows {
                        ...
                        image
                        latestChapter
                        genres${includeAuthor ? `
                        author` : ""}
                      }
                    }
                  }`;
    }
    async fetchSearchRows(params, includeAuthor = true) {
      try {
        const data = await this.executeGraphQL(this.buildSearchQuery({
          ...params,
          includeAuthor
        }));
        if (!data.search?.rows) {
          throw new Error("Failed to parse search rows from Mangahub response.");
        }
        return data.search.rows;
      } catch (e) {
        if (includeAuthor && /Cannot query field/i.test(`${e}`)) {
          return await this.fetchSearchRows(params, false);
        }
        throw e;
      }
    }
    ...
    async getSearchTags() {
      ...
      if (data.genres?.length == 0) throw new Error("Failed to parse genres property from data object!");
      const arrayTags = [];
      for (const genre of data.genres) {
        const label = decode(genre.title ?? "").trim();
        if (!genre.slug || !label) continue;
        arrayTags.push({ id: genre.slug, label });
      }
      arrayTags.sort((a, b) => a.label.localeCompare(b.label));
    ...
    async getSearchResults(query, metadata) {
      const offset = metadata?.offset ?? 0;
      const includedGenres = query?.includedTags?.map((x) => x.id) ?? [];
      const excludedGenres = query?.excludedTags?.map((x) => x.id) ?? [];
      const rawTitle = query?.title?.trim() ?? "";
      const authorSearch = /^(author|by)\s*:/i.test(rawTitle);
      const searchTitle = authorSearch ? rawTitle.replace(/^(author|by)\s*:/i, "").trim() : rawTitle;
      if (authorSearch && !searchTitle) {
        return App.createPagedResults({
          results: []
        });
      }
      let rows = [];
      if (searchTitle) {
        rows = rows.concat(await this.fetchSearchRows({
          title: searchTitle,
          offset,
          alt: false
        }));
      ...
      } else if (includedGenres.length > 1) {
        const genreRows = await Promise.all(includedGenres.map((genre) => this.fetchSearchRows({
          title: "",
          genre,
          offset,
          alt: false
        ...
        rows = genreRows[0] ?? [];
        rows = rows.filter((row) => genreRows.every((currentRows) => currentRows.some((currentRow) => currentRow.slug == row.slug)));
        if (rows.length == 0) {
      ...
      let manga = parseSearch(rows, {
        includedGenres,
        excludedGenres,
        authorQuery: authorSearch ? searchTitle : ""
      });
      const seen = /* @__PURE__ */ new Set();
      manga = manga.filter((item) => {
        const duplicate = seen.has(item.mangaId);
